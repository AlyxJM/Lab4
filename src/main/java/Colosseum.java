...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        int tempHitPoints = sc.nextInt();
        while (1 > tempHitPoints || tempHitPoints > MAX_HIT_POINTS) {
            System.out.println("Sorry. Hit points must be between 1 and 50: " + tempHitPoints);
            tempHitPoints = sc.nextInt();
        }
        tempPokemon.hitPoints = tempHitPoints;
        System.out.println("Split fifty points between attack level and defense level."
                + "\nEnter your attack level (1-49):");
        int tempAttackLevel = sc.nextInt();
        while (1 > tempAttackLevel || tempAttackLevel > MAX_ATTACK_LEVEL) {
            System.out.println("Sorry. The attack level must be between 1 and 49: " + tempAttackLevel);
            tempAttackLevel = sc.nextInt();
        }
        int levelLeft = MAX_ATTACK_LEVEL - tempAttackLevel;
        System.out.println("Enter your defense level (1-" + levelLeft + "):");
        int tempDefenseLevel = sc.nextInt();
        while (1 > tempDefenseLevel || tempDefenseLevel > levelLeft) {
            System.out.println("Sorry. The attack level must be between 1 and " + levelLeft + ": " + tempDefenseLevel);
            tempDefenseLevel = sc.nextInt();
        }
        return tempPokemon;
    }
